...
| **The app's version** (M10) | **`1.0.0`, one number in `package.json`, `Cargo.toml` and `tauri.conf.json`, shown as *Έκδοση εφαρμογής* beside the schema version; tag `v1.0.0`** on the merge commit after the product owner's sign-off. **Now `1.0.2`** (tag `v1.0.2`, 2026-09-26). `1.0.1`: per-lesson attendance, schema 11. `1.0.2`: renamed «Ημερολόγιο Εκπαιδευτικού» (English: Teacher Planner), no schema change | A Rust test fails if the three manifests disagree. The version on screen is the Rust side's own `CARGO_PKG_VERSION`, so it is the binary's and not a copy |
